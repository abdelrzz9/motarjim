...
impl FlutterGenerator {
    ...
    fn emit_heading(&self, _tree: &IrTree, _node: &IrNode, level: u32, w: &mut CodeWriter) {
        ...
        w.write_line(&format!("style: TextStyle(fontSize: {size}, fontWeight: FontWeight.bold),"));
        w.dedent();
        w.write_line("),");
    }
    ...
    fn emit_main_axis_alignment(&self, w: &mut CodeWriter, justify: &Option<motarjim_ast::style::JustifyContent>) {
        if let Some(ref jc) = justify {
            let value = match jc {
                ...
                motarjim_ast::style::JustifyContent::SpaceEvenly => {
    ...
    fn emit_cross_axis_alignment(&self, w: &mut CodeWriter, align: &Option<motarjim_ast::style::AlignItems>) {
    ...
    fn format_edge_values_dart(&self, ev: &EdgeValues) -> String {
        ...
        } else if (top - bottom).abs() < f64::EPSILON
            && (right - left).abs() < f64::EPSILON
        {
            format!(
                "const EdgeInsets.symmetric(vertical: {top}, horizontal: {right})"
            )
        } else {
            format!(
...
mod tests {
    ...
    fn test_column_and_row() {
        let tree = make_tree(
            vec![
                make_node(0, SemanticIr::Root, LayoutIr::FlexColumn, vec![1], None),
                make_node(1, SemanticIr::Column, LayoutIr::FlexColumn, vec![2], Some(0)),
    ...
    fn test_hero_section() {
        let tree = make_tree(
            vec![
                make_node(0, SemanticIr::Root, LayoutIr::FlexColumn, vec![1], None),
                make_node(1, SemanticIr::HeroSection, LayoutIr::Stack, vec![2], Some(0)),
                make_node(2, SemanticIr::Heading { level: 1 }, LayoutIr::Static, vec![], Some(1)),
            ],
            0,
        );
        let gen = FlutterGenerator::new();
