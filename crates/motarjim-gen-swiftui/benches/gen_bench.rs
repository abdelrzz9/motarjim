...
fn bench_generate_swiftui_small(c: &mut Criterion) {
    let tree = make_small_tree();
    let gen = SwiftUIGenerator::new();

    c.bench_function("generate_swiftui_small", |b| {
        b.iter(|| gen.generate(black_box(&tree)));
    });
}

fn bench_generate_swiftui_medium(c: &mut Criterion) {
    let tree = make_medium_tree();
    let gen = SwiftUIGenerator::new();

    c.bench_function("generate_swiftui_medium", |b| {
        b.iter(|| gen.generate(black_box(&tree)));
    });
}

criterion_group!(benches, bench_generate_swiftui_small, bench_generate_swiftui_medium);
