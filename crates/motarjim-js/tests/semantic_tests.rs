...
fn test_duplicate_let_declaration() {
    ...
    assert_eq!(diags[0].code.number, JsDiagnosticCode::JS_DUPLICATE_DECLARATION.0);
}
...
fn test_undeclared_variable_warning() {
    ...
    assert_eq!(diags.len(), 1);
    assert_eq!(diags[0].code.number, JsDiagnosticCode::JS_UNDECLARED_VARIABLE.0);
}
...
fn test_continue_outside_loop() {
    ...
    assert_eq!(diags.len(), 1);
    assert_eq!(diags[0].code.number, JsDiagnosticCode::JS_ILLEGAL_CONTINUE.0);
}

#[test]
fn test_await_outside_async_function() {
    let diags = analyze("await promise;");
    assert_eq!(diags.len(), 1);
...
fn test_destructuring_declares_bindings() {
    let diags = analyze("const { a, b } = obj; console.log(a + b);");
    assert!(diags.is_empty());
}

#[test]
fn test_destructuring_array_declares() {
    let diags = analyze("const [x, y] = arr; console.log(x, y);");
    assert!(diags.is_empty());
}
